...
/**
 * Baseado no {@link PageView}
 * 
 * @author leonardo.otto
 * 
 */
public class PageViewWrap extends BaseView {
	...
	public PageViewWrap(Context context) {
		super(context);
	}

	public PageViewWrap(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
	...
	protected void doMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		if (imgSize) {
			...
			for (int i = 0; i < pageSize; i++) {
				if (imgOn != null) {
					componentSize += imgOn.getWidth() + 10;
				} else {
					componentSize += 16;
				}
			}
			
			int w = getMeasurement(widthMeasureSpec, componentSize);
